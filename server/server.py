...
class Server(object):
    ...
    def __init__(self, config):# 初始化服务器类，接收配置参数。
        self.config = config

    # Set up server
    def boot(self):# 设置服务器，加载数据，模型以及创建客户端。
    ...
    def load_data(self):
        ...
        # Set up data loader
        self.loader = {
            'basic': updated_load_data.Loader(config, generator),
            'bias': updated_load_data.BiasLoader(config, generator),
            'shard': updated_load_data.ShardLoader(config, generator)
        }[self.config.loader]
